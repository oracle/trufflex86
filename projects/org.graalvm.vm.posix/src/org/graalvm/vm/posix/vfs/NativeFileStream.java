            ...
            switch (rdwr) {
                ...
                case O_WRONLY:
                    ...
                    if (path.toFile().exists() && Posix.WARN_ON_FILE_DELETE) {
                        log.log(Levels.WARNING, String.format("opening existing file '%s' in O_WRONLY mode!", path.toString()));
                    }
                ...
                case O_RDWR:
                    ...
                    if (path.toFile().exists() && Posix.WARN_ON_FILE_DELETE) {
                        log.log(Levels.WARNING, String.format("opening existing file '%s' in O_RDWR mode!", path.toString()));
                    }
                    file = new RandomAccessFile(path.toFile(), "rw");
                    r = true;
                    w = true;
    ...
    public PosixPointer mmap(long size, int prot, int flags, long off) throws PosixException {
        ...
        try {
            ...
            if (BitTest.test(flags, Mman.MAP_PRIVATE) && BitTest.test(prot, Mman.PROT_WRITE)) {
                ...
                return new ByteBufferPosixPointer(buf, 0, roundToPageSize(size), realsize, path.toAbsolutePath().normalize().toString());
