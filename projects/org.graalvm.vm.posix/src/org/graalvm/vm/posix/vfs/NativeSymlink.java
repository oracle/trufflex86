...
public class NativeSymlink extends VFSSymlink {
    ...
    public VFSEntry getTarget() throws PosixException {
        try {
            ...
            BasicFileAttributes info = Files.getFileAttributeView(p, BasicFileAttributeView.class, NOFOLLOW_LINKS).readAttributes();
            if (info.isDirectory()) {
    ...
    public long size() throws PosixException {
        try {
            return Files.getFileAttributeView(path, BasicFileAttributeView.class, NOFOLLOW_LINKS).readAttributes().size();
        } catch (IOException e) {
            throw new PosixException(Errno.EIO);
        }
    }
